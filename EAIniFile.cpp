...
{
    mSectionPositionMap.getAllocator().setAllocator(pCoreAllocator);
    mSectionNameMap.getAllocator().setAllocator(pCoreAllocator);
}
...

    {
        ...
        for(SectionLowerToPositionMap::iterator it(mSectionPositionMap.begin()); it != mSectionPositionMap.end(); ++it)
        {
            ...
            if(it1 == mSectionNameMap.end())
            {
                ...
                sectionList.pushBack(sSection);
            }
            else
                sectionList.pushBack((*it1).second);
            nSectionCount++;
        }
                        ...
                        if(sValue[0] != ';') // If not a comment line...
                        {
                            ...
                            if(nEquals != String16::npos) // If this line was a key/value line
                            {
                                ...
                                if(!sKey.empty())
                                {
                                    ...
                                    data_list.pushBack(DataList_t::value_type(sKey, sValue));
                                    nEntryCount++;
                                }
                            }
                        }
